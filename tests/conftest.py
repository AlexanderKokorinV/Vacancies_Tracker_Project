...
def db_setup() -> Generator[psycopg2.extensions.connection, None, None]:
    ...
    with conn.cursor() as cur:
        ...
        cur.execute(
            "INSERT INTO vacancies (employer_id, title, salary_from, salary_to) VALUES (1, 'Python Dev', 100, 200)"
        )
    conn.commit()
    conn.close()
...
def test_db_config() -> dict[str, Any]:
    ...
    return {
        ...
        "params": {"user": "postgres", "password": os.getenv("DB_PASSWORD"), "host": "localhost", "port": 5432},
    }
...
def db_connection(
    db_setup: None,
) -> Generator[psycopg2.extensions.connection, None, None]:  # Используем созданную ранее фикстуру базы
